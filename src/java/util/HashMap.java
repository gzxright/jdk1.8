...
public class HashMap<K,V> extends AbstractMap<K,V>
     ...
     */
    /*不管增加、删除、查找键值对，定位到哈希桶数组的位置都是很关键的第一步。前面说过 HashMap 的数据结构是“数组+链表+红黑树”的结合，
    所以我们当然希望这个 HashMap 里面的元素位置尽量分布均匀些，尽量使得每个位置上的元素数量只有一个，那么当我们用 hash 算法求得这个位置的时候，
    马上就可以知道对应位置的元素就是我们要的，不用遍历链表/红黑树，大大优化了查询的效率。HashMap 定位数组索引位置，直接决定了 hash 方法的离散性能
    整个过程本质上就是三步：
    1拿到 key 的 hashCode 值
    2将 hashCode 的高位参与运算，重新计算 hash 值
    3将计算出来的 hash 值与 (table.length - 1) 进行 & 运算
    */
    //定位哈希桶数组索引位置 1
    //计算key的hash值
     ...
     */
    /*在HashMap的底层存在着一个名字为table的Entry数组，在实例化HashMap的时候，会输入两个参数，一个是 int initCapacity（初始化数组大小，默认值是16），
    一个是float loadFactor（负载因子，默认值是0.75），首先会根据initCapacity计算出一个大于或者等于initCapacity且为2的幂的值capacity，例如initCapacity为15，
    那么capacity就会为16，还会算出一个临界值threshold，也就是capacity * loadFactor
    */
     ...
     */
    /*创建完HashMap之后，下面来看put方法，首先会根据key值计算出其HashCode值，然后在通过一个indexFor方法计算出此元素该存放于table数组的哪个数组之中
    （我猜想可能是通过对table.length的值取余的操作计算出来的），再检测此table的此坐标位置的entry链是否存在此key或者此key值，若存在，则更新此元素的value值
    */
    ...
    static final class TreeNode<K,V> extends LinkedHashMap.Entry<K,V> {
        ...
        final HashMap.TreeNode<K,V> getTreeNode(int h, Object k) {
            // 红黑树的查找目标节点方法一 1.首先找到红黑树的根节点；2.使用根节点调用find方法
